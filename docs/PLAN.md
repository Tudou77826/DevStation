...
供应商配置只能增量合并带 DevStation 标识的配置项，必须可检查、可重复执行、可卸载，不覆盖用户已有 Hook/Plugin。启用集成需要用户明确操作；集成缺失或损坏时，终端与原生恢复能力继续可用。
...
- `[L3]` 明确展示 Plugin 未安装、事件过期和仅终端可用三种降级状态。

验收：OpenCode 状态来自真实事件；没有 Plugin 时仍可运行和恢复，但 UI 不伪造 Agent 状态。
