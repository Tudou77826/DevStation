...
状态：已完成。Registry 在注册时拒绝不兼容的 Descriptor/设置 Schema；通用契约测试覆盖内置 Adapter 的结构化 argv、能力降级和引用校验。诊断结果在 Main 边界脱敏，事件收件箱已验证跨数据库重启的回放与去重；确定性 E2E 使用仅测试环境注册的 Test Agent，真实 OpenCode/Chrys smoke 继续显式启用。
...
M4 已完成，后续新增 Agent 按[适配器接入指南](./ADDING_CODING_AGENT.md)和现有契约门禁实施。
